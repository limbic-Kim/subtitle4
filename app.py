...
class WhisperInference(BaseInterface):
    ...
    def transcribe_file(self,
        ...
        try:
            ...
            for fileobj in fileobjs:
                ...
                # print(f'Loading Audio.. fileobj = {fileobj}, fileobj.name= {fileobj.name}')
    ...
    def check_and_split_audio(file_path: str, temp_folder: str, max_duration: float = 7200.0,  # 60.0, # 7200.0, #todo
        ...
        audio_extensions = ['.mp3', '.wav', '.m4a'] #todo : mkv, webm까지 해서 video 에 추가
        clip = None
        duration = 0
...
def parse_arguments():
    ...
    parser.add_argument('--force_skip_translation', action='store_true', 
                        help='Force translation even if the detected language is Korean') # 옵션을 주면 True, 안쓰면 False
    return parser.parse_args()


def process_files(input_file_dir, model, lang, subformat, translate):
    whisper_inference = WhisperInference()

    # Find all audio/video files in the given input directory
    file_extensions = ['*.mp3', '*.webm', '*.wav', '*.m4a', '*.mp4']
...
def main():
    ...
    if args.mode == 'files':
        if not os.path.isdir(args.input_file_dir):
            ...
            sys.exit(1)
        process_files(args.input_file_dir, args.model, args.lang, args.subformat, args.translate)
        if not args.force_skip_translation and args.lang != "Korean":
        ...
        if not args.youtube_links_input:
            ...
            sys.exit(1)
        process_youtube_links(args.youtube_links_input, args.model, args.lang, args.subformat, args.translate)
